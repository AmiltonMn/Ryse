      ...
      <main className={style.main}>
        <form className="text-white h-1/2 w-96 bg-zinc-800 flex flex-col items-center gap-7 p-10">
          ...
          <div className="flex flex-col items-center gap-2  text-black">
            <input className="rounded p-1 " placeholder="Name" value={name} onChange={(e) => setName(e.target.value)} />
            <input type="email" className="rounded p-1" placeholder="Email" value={email} onChange={(e) => setEmail(e.target.value)} />
            <input className="rounded p-1" placeholder="EDV" value={EDV} onChange={(e) => setEDV(e.target.value)} />
            <input type="password" className="rounded p-1" placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} />
          </div>
          <button type="button" onClick={async() => await fetch(name,email,EDV,password)} className="bg-white text-black p-2 rounded w-1/2">Sign Up</button>
