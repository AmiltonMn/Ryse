'use client'

import { CardForum } from "@/components/cardForum";
import { Menu } from "@/components/menu";
import { Submenu } from "@/components/submenu";
import { ROUTES } from "@/constants/routes";
import Link from "next/link";
import Image from "next/image";

import { CardAnswer } from "@/components/cardAnswer";
import iconProfile from "@/assets/user.png"
import search from "@/assets/lupaBlack.png"
import iconMore from "@/assets/mais.png";
import { useEffect, useState } from "react";
import { api } from "@/constants/api";

const styles = {
    chat: "p-2 mt-6 rounded-[10px] border-[#4B4B4B] border-[0.5px] w-full "
}

interface QuestionData {
    question: {
        idQuestion: number;
        user: string;
        title: string;
        text: string;
        topic: string;
        date: string;
        isOwner: boolean;
        answersCount: number;
    };
    answers: {
        idAnswer: number;
        user: string;
        date: string;
        text: string;
        likes: number;
        liked: boolean;
        verified: boolean;
        isOwner: boolean;
    }[];
}

export default function Question() {

    const [newAnswer, setNewAnswer] = useState<string>("");
    const [data, setData] = useState<QuestionData>();
    let idQuestion = localStorage.getItem("question");

    useEffect(() => {

        api.get(
            `/forum/question/${idQuestion}`, 
            {
                headers: {
                    "Authorization": localStorage.getItem("token")
                }
            }
        ).then((res) => {
            setData(res.data)
        })
        .catch((e) => {})
    }, [])

    const handleNewAnswer = async () => {

        if(!newAnswer)
            return 

        await api.post(`/forum/question/${idQuestion}/answer`,
            {
                "text": newAnswer
            },
            {
                headers: {
                    "Authorization": localStorage.getItem("token")
                }
            })
            .then((res) => {
                alert("Answer sended")
                window.location.reload()   
            })
            .catch((e) => {
                console.log(e)
            })
    }

    return (
        <div>
            <Menu title={"Ryse"}/>
            <Submenu home={"Home"} chats={"Chats"} newGroup={"New group"} myGroup={"My groups"} chatPrincipal1={"Chat 1"} chatPrincipal2={"Chat 2"} chatPrincipal3={"Chat 3"} newIdea={"New idea"} ideas={"Ideas"} hardSkills={"Hard Skills"} events={"Events"} news={"News"} />
            <div className="pt-36 mb-10 pl-[300px] flex">
                <div className="w-[97%]">
                    <div className="flex justify-between items-center">
                        <h2 className="text-white font-bold text-[16px] mb-3">{data?.question.title}</h2>
                        <div className="flex text-white">
                            <Image src={iconProfile} alt="ícone notificação" className="w-9 h-9 rounded-t-3xl m-2 mr-6" width={1000} height={1000}/>
                            <div className="mb-4">
                                <h4>{data?.question.user}</h4>
                                <p className="text-[12px]">{data?.question.date}</p>
                            </div>
                        </div>
                    </div>
                    <hr/>
                    <div className="bg-[#242424] rounded-[10px] mt-12  text-white p-12 text-[16px]">
                        <p>{data?.question.text}</p>
                    </div>
                    <div className="flex flex-col justify-center text-white mt-12 m-16">
                        <div className="flex justify-between">
                            <h3 className="font-bold text-[16px]">Answers</h3>
                            <div className="flex items-center">
                                <p className="pr-12">{data?.question.answersCount} answers</p>
                                <div>
                                    <h3 className="text-[#F41C54] font-bold bg-[#D9D9D9] p-1 pr-5 pl-5 rounded-[10px]">{data?.question.topic}</h3>
                                </div>
                            </div>
                        </div>
                        <div>
                            {data?.answers.map((answer) => 
                            (
                                <CardAnswer key={answer.idAnswer} idAnswer={answer.idAnswer} userPhoto={iconProfile.src} username={answer.user} date={answer.date} answer={answer.text} likes={answer.likes} liked ={answer.liked} verified={answer.verified}/>
                            ))}
                            {/* // <CardAnswer userPhoto={iconProfile.src} username={"Ingrid Rocha"} date={"12/12/2023"} answer={"aaaaaaaaaa"}/>
                            // <CardAnswer userPhoto={iconProfile.src} username={"Ingrid Rocha"} date={"12/12/2023"} answer={"aaaaaaaaaa"}/>
                            // <CardAnswer userPhoto={iconProfile.src} username={"Ingrid Rocha"} date={"12/12/2023"} answer={"aaaaaaaaaa"}/> */}
                            <div className="bg-[#242424] fixed bottom-2 rounded-[10px] mt-16 w-[75%] text-white border-[#f8f8f8] border-[0.3px]">
                                <div className="text-[20px] flex justify-center p-5">
                                    <Image src={iconProfile.src} alt="ícone notificação" className="w-9 h-9 rounded-t-3xl mr-6" width={1000} height={1000}/>
                                    <input type="text" className="w-[97%] text-[12px] rounded-[10px] text-black p-2 h-10 mr-2 pl-4" placeholder="Post answer" value={newAnswer} onChange={(e) => { setNewAnswer(e.target.value) }}></input>
                                    <button onClick={handleNewAnswer} className="text-white w-[150px] h-10 text-[13px] hover:text-gray-500 black transition easy-in-out bg-[#F41C54] rounded-[10px] flex items-center justify-center">Add answer</button>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

        </div>
    );
}